...
public class SpawnController : MonoBehaviour
{
    ...
    public void SpawnNMESaiyen()
    {
        ...
        {
            int random = Random.Range(0, spawnPoints.childCount);
            Instantiate(enemyPrefab2, spawnPoints.GetChild(random).transform.position, Quaternion.identity);
            nbrNME++;
        }
    }
    ...
    public void SpawnNMEMIX()
    {
        for (int i = 0; i < Random.Range(1, HowmanyNME); i++)
        {
            ...
            {
                int random = Random.Range(0, spawnPoints.childCount);
                Instantiate(enemyPrefab1, spawnPoints.GetChild(random).transform.position, Quaternion.identity);
            }
            else
            {
                int random = Random.Range(0, spawnPoints.childCount);
